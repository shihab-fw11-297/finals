...
export const MODE_CONFIG: Record<EntryMode, {
  ...
  V2_DEFAULT: {
    label: "V2 Default",
    description: "Strategy-owned V2 rules; legacy mode thresholds are not used for signal generation.",
    requiredSetupScore: 0,
    requiredSignalScore: 0,
    minRR: 1.5,
    extensionAtr: 3,
    confirmationWindowCandles: 0,
    allowNeutralHTF: true,
    rejectOppositeHTF: false,
    allowTriggerAsConfirmation: false,
    sessionFilterMode: "WARNING_ONLY",
    volatilityFilterMode: "WARNING_ONLY",
    reversalRiskMode: "WARNING_ONLY",
  },
...
const MODE_RULES: Record<EntryMode, { setupScore: number; signalScore: number; rr: number; extensionAtr: number; confirmationWindowCandles?: number }> = {
  ...
  V2_DEFAULT: { setupScore: 0, signalScore: 0, rr: 1.5, extensionAtr: 3, confirmationWindowCandles: 0 },
};
...
export function evaluateSetup(
  ...
  const directionBias = directionToBias(direction);
  const htfBias = directionToBias(evaluationInput.context.htfBias.bias);
  const htfAligned = htfBias === direction;
  const oppositeHtf = directionBias === "BULLISH" ? "BEARISH" : "BULLISH";
  const htfOpposed = htfBias === oppositeHtf;
