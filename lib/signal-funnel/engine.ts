...
export function buildSignalFunnel(input: {
  ...
  const isV2Goldmine = input.signals.audit.activeEngine === "V2_GOLDMINE";
  const counts: SignalFunnelCounts = {
    ...
    triggerSetupCount: isV2Goldmine ? input.signals.audit.triggerSetupsFound : input.setups.audit.triggerCount,
  ...
  return {
    counts,
    zeroCountExplanations: explainZeroCounts(counts, isV2Goldmine),
    ...
    triggerDiagnostics: isV2Goldmine
      ? buildV2TriggerDiagnostics(input.signals)
      : buildTriggerDiagnostics(input.setups, input.signals, candles),
    goldUnits: buildGoldUnitDiagnostics(candles, input.structure.atr, input.signals.signals),
    blocker: identifyBlocker(counts, isV2Goldmine),
  };
}

function explainZeroCounts(counts: SignalFunnelCounts, isV2Goldmine = false): string[] {
  ...
  if (counts.sweepCount === 0 && !isV2Goldmine) explanations.push("No sweeps detected. Liquidity sweep logic is the likely blocker.");
  if (counts.displacementCount === 0) explanations.push("No displacement candles detected. Momentum threshold may be too strict.");
  if (counts.triggerSetupCount === 0) explanations.push(isV2Goldmine ? "No V2 Asian high/low sweeps were detected after a valid Asian range." : "No TRIGGER setups. Phase 4 setup scanner is blocking before Phase 5 can create signals.");
  if (counts.triggerSetupCount > 0 && counts.confirmedBuyCount + counts.confirmedSellCount === 0) explanations.push(isV2Goldmine ? "V2 sweeps exist, but no confirmed Goldmine signals. Confirmation, displacement/MSS, TP, RR, or score filters are blocking." : "TRIGGER setups exist, but no confirmed signals. Phase 5 confirmation, SL/TP, RR, or context filters are blocking.");
...
function buildV2TriggerDiagnostics(signals: EntryEngineResult): TriggerDiagnosticsRow[] {
  const signalBySetup = new Map(signals.signals.map((signal) => [signal.sourceSetupId, signal]));
  return signals.candidateDebug.slice(-20).map((debug) => {
    const signal = signalBySetup.get(debug.setupId);
    return {
      setupId: debug.setupId,
      direction: debug.directionBias ?? signal?.direction ?? "NEUTRAL",
      setupType: "LIQUIDITY_SWEEP_REVERSAL",
      score: debug.signalScore ?? debug.finalSignalScore ?? signal?.score ?? 0,
      requiredSetupScore: 0,
      finalSignalScore: debug.finalSignalScore ?? signal?.score ?? null,
      requiredSignalScore: debug.requiredSignalScore,
      confirmationStatus: debug.confirmationStatus,
      confirmationWindowRemaining: debug.confirmationWindowRemaining,
      entryCandidate: signal?.entryPrice ?? null,
      stopLoss: signal?.stopLoss ?? null,
      takeProfit: signal?.takeProfit ?? null,
      rr: debug.rr ?? signal?.rr ?? null,
      rejected: !signal,
      rejectionReason: debug.rejectionReason,
      nextRequiredAction: debug.nextRequiredAction,
    };
  });
}
...
function buildTriggerDiagnostics(setups: SetupScannerResult, signals: EntryEngineResult, candles: Candle[]): TriggerDiagnosticsRow[] {
...
function identifyBlocker(counts: SignalFunnelCounts, isV2Goldmine = false): string {
  ...
  if (isV2Goldmine) {
    if (counts.triggerSetupCount === 0) return "V2 Goldmine has no Asian high/low sweep after a valid Asian range.";
    return "V2 Goldmine confirmation, displacement/MSS, TP/RR, or score rules are blocking.";
  }
