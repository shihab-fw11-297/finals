...
import type { RejectedSetup, TradeSignal, V2AsianRangeType } from "../entry-engine/types";
...
import { ACTIVE_SIGNAL_ENGINE } from "../v2-signal-engine/config";
import type {
  BacktestInput,
  BacktestResult,
  BacktestSettings,
  ...
  CalibrationResult,
  CalibrationSettings,
  IntermarketMacroBacktestComparison,
  IntermarketMacroBacktestGradeRow,
  NoFutureValidation,
  PerformanceMetrics,
  MasterBacktestComparison,
  InstitutionalBacktestComparison,
  OptionalMasterBacktestComparison,
  PropFirmResult,
  PropFirmSettings,
  RejectionAnalytics,
  RobustnessResult,
...
export function runBacktest(input: BacktestInput): BacktestResult {
  ...
  const signals = input.signals
    .filter((signal) => signal.engine === ACTIVE_SIGNAL_ENGINE)
    .filter(isConfirmedBacktestSignal)
    ...
    .filter((signal) => !settings.strategyFilter || settings.strategyFilter === "ALL" || (signal.strategyId ?? "LEGACY") === settings.strategyFilter)
  ...
  const result: BacktestResult = {
    ...
    breakdowns: {
      ...
      byKillzone: breakdown(trades, (trade) => trade.killzoneName ?? trade.session),
...
export function runMasterBacktestComparison(input: BacktestInput, selection: MasterSignalSelectionResult): MasterBacktestComparison {
  const raw = runBacktest({ ...input, signals: selection.rawSignals });
  const master = runBacktest({ ...input, signals: selection.finalSignals });
  return {
    raw,
    master,
    rawTotalTrades: raw.trades.length,
    masterSelectedTrades: master.trades.length,
    suppressedDuplicates: selection.suppressedSignals.length,
    rawWinRate: raw.metrics.winRate,
    masterWinRate: master.metrics.winRate,
    rawProfitFactor: raw.metrics.profitFactor,
    masterProfitFactor: master.metrics.profitFactor,
    rawDrawdown: raw.metrics.maxDrawdown,
    masterDrawdown: master.metrics.maxDrawdown,
    expectancyChange: round(master.metrics.expectancy - raw.metrics.expectancy, 3),
  };
}

export function runOptionalMasterBacktestComparison(
  input: BacktestInput,
  selection: OptionalMasterSelectionResult,
): OptionalMasterBacktestComparison {
  const raw = runBacktest({ ...input, signals: selection.rawSignals });
  const master = runBacktest({ ...input, signals: selection.finalSignals });
  return {
    ...
    masterSelectedTrades: master.trades.length,
    suppressedDuplicates: selection.suppressedSignals.length,
    conflictCount: selection.conflictSignals.length,
    rawWinRate: raw.metrics.winRate,
    masterWinRate: master.metrics.winRate,
    rawProfitFactor: raw.metrics.profitFactor,
    masterProfitFactor: master.metrics.profitFactor,
    rawExpectancy: raw.metrics.expectancy,
    masterExpectancy: master.metrics.expectancy,
    rawDrawdown: raw.metrics.maxDrawdown,
    masterDrawdown: master.metrics.maxDrawdown,
    expectancyChange: round(master.metrics.expectancy - raw.metrics.expectancy, 3),
  };
...
export function runIntermarketMacroBacktestComparison(input: BacktestInput): IntermarketMacroBacktestComparison {
  const raw = runBacktest(input);
  const scoreOnly = runBacktest({ ...input, signals: input.signals });
  const blockingSignals = input.signals.filter((signal) => !signal.intermarket?.shouldBlock);
  const blocking = runBacktest({ ...input, signals: blockingSignals });
  const blockedSignals = input.signals.filter((signal) => signal.intermarket?.shouldBlock);
  const rawTradesBySignalId = new Map(raw.trades.map((trade) => [trade.signalId, trade]));
  const blockedTrades = blockedSignals
    .map((signal) => rawTradesBySignalId.get(signal.id))
    .filter((trade): trade is BacktestTrade => Boolean(trade));
  const gradeRows = macroGradeRows(raw.trades);
  const blockedRows = macroGradeRows(blockedTrades);
  const macroMissing = input.signals.filter((signal) => !signal.intermarket || signal.intermarket.macroGrade === "UNKNOWN").length;

  return {
    raw,
    scoreOnly,
    blocking,
    rawTotalSignals: input.signals.length,
    scoreOnlySignals: input.signals.length,
    blockingSignals: blockingSignals.length,
    signalsWithMacroA: input.signals.filter((signal) => signal.intermarket?.macroGrade === "A").length,
    signalsWithMacroB: input.signals.filter((signal) => signal.intermarket?.macroGrade === "B").length,
    macroConflictSignals: input.signals.filter((signal) => signal.intermarket?.macroGrade === "CONFLICT").length,
    tradesBlocked: blockedTrades.length,
    rawDrawdown: raw.metrics.maxDrawdown,
    blockingDrawdown: blocking.metrics.maxDrawdown,
    drawdownChange: round(blocking.metrics.maxDrawdown - raw.metrics.maxDrawdown, 2),
    rawExpectancy: raw.metrics.expectancy,
    scoreOnlyExpectancy: scoreOnly.metrics.expectancy,
    blockingExpectancy: blocking.metrics.expectancy,
    expectancyChange: round(blocking.metrics.expectancy - raw.metrics.expectancy, 3),
    winRateByMacroGrade: gradeRows,
    expectancyByMacroGrade: gradeRows,
    blockedTradeOutcomes: blockedRows,
    notes: [
      "RAW_STRATEGY keeps all strategy signals without a macro filter.",
      "MACRO_SCORE_ONLY keeps all signals and reports outcomes by attached macro grade.",
      "MACRO_BLOCKING removes only signals already marked shouldBlock by the intermarket gate.",
      macroMissing
        ? `${macroMissing} signal${macroMissing === 1 ? "" : "s"} had missing or unknown macro data and remained eligible.`
        : "All compared signals carried usable macro grades.",
    ],
  };
}

export function runInstitutionalBacktestComparison(
  input: BacktestInput,
  selection: InstitutionalMasterSelectionResult,
): InstitutionalBacktestComparison {
  const research = runBacktest({ ...input, signals: selection.researchSignals });
  const production = runBacktest({ ...input, signals: selection.finalSignals });
  return {
    research,
    production,
    researchRawTrades: research.trades.length,
    productionMasterTrades: production.trades.length,
    productionRejectedSignals: selection.rejectedSignals.length + selection.suppressedSignals.length,
    researchWinRate: research.metrics.winRate,
    productionWinRate: production.metrics.winRate,
    researchProfitFactor: research.metrics.profitFactor,
    productionProfitFactor: production.metrics.profitFactor,
    researchDrawdown: research.metrics.maxDrawdown,
    productionDrawdown: production.metrics.maxDrawdown,
    expectancyChange: round(production.metrics.expectancy - research.metrics.expectancy, 3),
  };
}
...
function tradeFrom(
  ...
  return {
    ...
    noFutureValidation,
    strategyId: signal.strategyId,
    killzoneName: signal.silverBullet?.killzoneName,
    retestDelay: signal.retest?.retestDelay,
    asianRangeType: signal.asianRange?.rangeType,
    largeAsianRange: signal.asianRange?.warnings.includes("WARNING_LARGE_ASIAN_RANGE") ?? false,
    macroGrade: signal.intermarket?.macroGrade,
    macroScore: signal.intermarket?.macroScore,
    macroGoldBias: signal.intermarket?.goldBias,
    macroBlockReason: signal.intermarket?.blockReason,
    macroWarnings: signal.intermarket?.warnings,
  };
}
...
export function calculatePerformanceMetrics(trades: BacktestTrade[], startingBalance = DEFAULT_SETTINGS.accountBalance): PerformanceMetrics {
  ...
  const breakoutTrades = trades.filter((t) => t.strategyId === "ASIAN_RANGE_BREAKOUT_RETEST");
  const breakoutWins = breakoutTrades.filter((t) => t.finalR > 0.05).length;
  const breakoutWinRate = breakoutTrades.length ? breakoutWins / breakoutTrades.length : undefined;

  const retestDelays = breakoutTrades
    .map((t) => t.retestDelay)
    .filter((d): d is number => d !== undefined);
  const averageRetestDelay = retestDelays.length ? mean(retestDelays) : undefined;
  const sessionRows = breakdownShallow(trades, (trade) => trade.session);
  const bestSession = sessionRows[0]?.key;
  const breakoutDirectionPerformance = breakoutTrades.length
    ? (["BUY", "SELL"] as const)
        .map((direction) => {
          const group = breakoutTrades.filter((trade) => (trade.direction === "BULLISH" ? "BUY" : "SELL") === direction);
          const wins = group.filter((trade) => trade.finalR > 0.05).length;
          return {
            direction,
            totalTrades: group.length,
            winRate: group.length ? roundPercent(wins / group.length) : 0,
            totalR: round(sumR(group), 3),
          };
        })
        .filter((row) => row.totalTrades > 0)
    : undefined;
  const completeRangeSignals = trades.filter((trade) => trade.asianRangeType === "COMPLETE").length;
  const partialRangeSignals = trades.filter((trade) => trade.asianRangeType === "PARTIAL").length;
  const fallbackRangeSignals = trades.filter((trade) => trade.asianRangeType === "FALLBACK").length;
  const largeRangeSignals = trades.filter((trade) => trade.largeAsianRange).length;
  const winRateByRangeType = (["COMPLETE", "PARTIAL", "FALLBACK"] as const)
    .map((rangeType: V2AsianRangeType) => {
      const group = trades.filter((trade) => trade.asianRangeType === rangeType);
      const rangeWins = group.filter((trade) => trade.finalR > 0.05).length;
      return {
        rangeType,
        totalTrades: group.length,
        winRate: group.length ? roundPercent(rangeWins / group.length) : 0,
      };
    })
    .filter((row) => row.totalTrades > 0);
  ...
  return {
    ...
    averageR: round(mean(trades.map((trade) => trade.finalR)), 3),
    ...
    bestTradeR: round(Math.max(0, ...trades.map((trade) => trade.finalR)), 3),
    worstTradeR: round(Math.min(0, ...trades.map((trade) => trade.finalR)), 3),
    ...
    breakoutWinRate: breakoutWinRate !== undefined ? roundPercent(breakoutWinRate) : undefined,
    averageRetestDelay: averageRetestDelay !== undefined ? round(averageRetestDelay, 2) : undefined,
    bestSession,
    breakoutDirectionPerformance,
    completeRangeSignals,
    partialRangeSignals,
    fallbackRangeSignals,
    largeRangeSignals,
    winRateByRangeType,
  };
}
...
function breakdownShallow(trades: BacktestTrade[], keyFor: (trade: BacktestTrade) => string): Array<{ key: string; totalTrades: number; totalR: number; winRate: number }> {
  const groups = new Map<string, BacktestTrade[]>();
  for (const trade of trades) {
    const key = keyFor(trade);
    groups.set(key, [...(groups.get(key) ?? []), trade]);
  ...
  return [...groups.entries()]
    ...
    .sort((a, b) => b.totalR - a.totalR || b.winRate - a.winRate || b.totalTrades - a.totalTrades);
}

function macroGradeRows(trades: BacktestTrade[]): IntermarketMacroBacktestGradeRow[] {
  const rows = breakdown(trades, (trade) => trade.macroGrade ?? "MISSING");
  return rows.map((row) => ({
    ...row,
    grade: row.key as IntermarketMacroBacktestGradeRow["grade"],
    blockedTrades: trades.filter((trade) => (trade.macroGrade ?? "MISSING") === row.key && trade.macroBlockReason).length,
  }));
}
...
function buildRejectionAnalytics(rejectedSetups: RejectedSetup[], confirmedSignalCount: number, tradeCount: number, candles: Candle[]): RejectionAnalytics {
  ...
  return {
    ...
    notes: histogram.length
      ...
      : ["No rejected V2 Goldmine setups were available for calibration analysis."],
  };
}
...
function runCalibration(input: BacktestInput, settings: BacktestSettings, candles: Candle[]): CalibrationResult[] {
  ...
  return sets.map((name) => {
    ...
    const filteredSignals = input.signals.filter((signal) =>
      signal.engine === ACTIVE_SIGNAL_ENGINE &&
      isConfirmedBacktestSignal(signal) &&
    ...
    return {
      ...
      notes: ["Best historical settings may overfit and fail live.", name === "relaxed settings" ? "Relaxed filters expose whether signal scarcity is filter-driven." : "Uses V2 Goldmine signals only; no legacy Phase 5 signals are included."],
    };
  });
}

function isConfirmedBacktestSignal(signal: TradeSignal): boolean {
  return signal.status === "CONFIRMED" && signal.type.startsWith("CONFIRMED_");
}
...
function buildCacheKey(input: BacktestInput, settings: BacktestSettings, candles: Candle[]): string {
  ...
  const signalSignature = input.signals
    .map((signal) => [
      signal.id,
      signal.status,
      signal.type,
      signal.confirmedAtIndex,
      signal.intermarket?.macroGrade ?? "NO_MACRO",
      signal.intermarket?.macroScore ?? "NO_SCORE",
      signal.intermarket?.shouldBlock ? "BLOCK" : "ALLOW",
    ].join("|"))
    .join(",");
  return [input.symbol, input.timeframe, input.startDate, input.endDate, settings.signalMode, candles.length, last?.timestamp ?? 0, input.signals.length, signalSignature, JSON.stringify(settings)].join(":");
}
