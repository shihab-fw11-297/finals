...
import type { MarketStructureSettings, MarketStructureResult, MomentumMarker, StructureMarker, SweepMarker, PressureMarker } from "./types";
...
describe("market structure engine", () => {
  ...
  it("separates close-through sweeps from wick sweeps", () => {
    ...
    const sweep = result.markers.find((marker) => marker.type === "SSL_SWEEP") as SweepMarker | undefined;
  ...
  it("does not attach future sweep evidence to earlier pressure markers", () => {
    ...
    const earlyBuyers = result.markers.find(
      (marker) => marker.type === "BUYERS" && marker.confirmedAtIndex === 3,
    ) as PressureMarker | undefined;
