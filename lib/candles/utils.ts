...
import { findEndIndexByTime, findStartIndexByTime } from "../chart-utils/visible-range";
...
export function filterCandlesByDateRange(
  ...
  const startIndex = findStartIndexByTime(candles, startTimestamp);
  const endIndex = findEndIndexByTime(candles, endTimestamp);
