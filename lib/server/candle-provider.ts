import type { CandleFetchRequest, Timeframe } from "../candles/types";
import { getTimeframeMs, parseDateToTimestamp } from "../candles/utils";
...
async function fetchFinageCandles(
  ...
  const candlesByChunk = await Promise.all(
    chunks.map(async (chunk) => {
      const url = new URL(
        `https://api.finage.co.uk/agg/${finageAssetClass(request.symbol)}/${formatFinageSymbol(
          request.symbol,
...
function finageAssetClass(symbol: string): "crypto" | "forex" {
  const normalizedSymbol = formatFinageSymbol(symbol);
  return normalizedSymbol === "BTCUSD" ? "crypto" : "forex";
}
