...
| Follow-through engine | `lib/v2-signal-engine/signal-follow-through-engine.ts` |
...
- Follow-through grade, target, obstacle, and invalidation overlays
...
- If `signal.followThrough` exists, marker text shows the frozen follow-through grade and estimated probability, such as `BUY A 78%` or `SELL A+ 86%`.
- Entry price line can be shown.
- Stop loss line can be shown.
- Take profit line can be shown.
- Follow-through target, nearest obstacle, and invalidation lines can be shown for selected/latest signals.
- Hover/select shows signal details.

## Phase 6: Signal Follow-Through Scoring

File:

```text
lib/v2-signal-engine/signal-follow-through-engine.ts
```

Phase 6 runs only after a strategy has already produced a confirmed raw `TradeSignal`.

It does not create a signal. It scores the signal that already exists.

Input:

```text
confirmed signal
closed candles known at confirmation
LTF/ITF/HTF context candles
ATR
session
market context
...
- HTF directional bias support or conflict.
- Liquidity runway from entry to the nearest target.
- Nearest target, usually BSL/SSL/session/swing/key liquidity.
- Nearest obstacle, such as supply, demand, FVG, OB, or HTF level.
- Target distance and obstacle distance in R.
- Displacement quality.
- Structure alignment.
- Entry quality.
- Session and volatility quality.
- Historical similar setup edge when data exists.
- Hard blockers and warnings.
- Frozen chart overlay metadata.

Grade mapping:

```text
A+    highest quality follow-through
A     strong follow-through
B     valid but not top quality
C     caution
AVOID low follow-through or hard blocker
```

Important no-repaint rule:

The follow-through score is calculated at the signal confirmation boundary. Future candles may be used later for post-trade analytics, but they must not change the original `followThroughScore`, `followThroughGrade`, target, obstacle, probability, or invalidation display.

Frozen proof fields:

```ts
{
  followThroughCalculatedAtIndex,
  candlesKnownAtCalculation,
  scoreFrozen,
  gradeFrozen,
  targetFrozen,
  obstacleFrozen,
